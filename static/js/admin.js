...
function buscarHistorial(pagina) {
    historialPagina = pagina || 1;
    const params = new URLSearchParams({
    ...
    fetch('/admin/historial_vehiculos?' + params)
        .then(r => r.json())
        .then(data => {
            if (!data.ok) return;
            ...
            if (!data.historial.length) {
                ...
                document.getElementById('paginacion-historial').innerHTML = '';
                return;
            }
            tbody.innerHTML = data.historial.map(h => `
                <tr>
                    ...
                    <td>${h.cliente}</td>
                    <td>${h.fecha_entrada} ${h.hora_entrada || ''}</td>
                    <td>${h.salio ? (h.fecha_salida || '') + ' ' + (h.hora_salida || '') : '<span class="badge badge-active">En cochera</span>'}</td>
                    <td>${h.dias_reales || h.dias}</td>
                    <td>S/ ${parseFloat(h.monto || 0).toFixed(2)}</td>
                    <td>S/ ${parseFloat(h.adelanto || 0).toFixed(2)}</td>
                    <td>${h.salio ? '<span class="badge badge-inactive">Salio</span>' : '<span class="badge badge-active">En cochera</span>'}</td>
                </tr>
            `).join('');

            let pagHtml = '';
            for (let i = 1; i <= data.total_paginas; i++) {
                pagHtml += `<button class="btn-sm ${i === data.pagina ? 'btn-primary' : 'btn-secondary'}" onclick="buscarHistorial(${i})">${i}</button> `;
            }
            pagHtml += `<span style="margin-left:1rem;">Total: ${data.total} registros</span>`;
            document.getElementById('paginacion-historial').innerHTML = pagHtml;
        });
}
...
function exportarExcel() {
    ...
    window.location.href = '/admin/exportar_historial?' + params;
}
